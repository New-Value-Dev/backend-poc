...
class DashboardRepository:
    ...
    @staticmethod
    def _scope(stmt: Select, project_id: int | None, accessible_project_ids=None) -> Select:
        """Document 가 걸린 쿼리에 프로젝트 스코프를 덧붙인다"""
        if project_id is not None:
            return stmt.where(Document.project_id == project_id)
        if accessible_project_ids is not None:
            return stmt.where(Document.project_id.in_(accessible_project_ids))
        return stmt
    ...
    def count_documents(
        ...
        stmt = self._scope(
            select(func.count()).select_from(Document), project_id, accessible_project_ids
        )
        if created_by is not None:
    ...
    def pipeline_counts(
        ...
        stmt = self._scope(stmt, project_id, accessible_project_ids)
        if created_by is not None:
            stmt = stmt.where(Document.created_by == created_by)
    ...
    def daily_version_counts(
        self, since: datetime, project_id: int | None = None, accessible_project_ids=None
    ) -> dict[date, int]:
        ...
        stmt = self._scope(stmt, project_id, accessible_project_ids)
        return {row.day: row.count for row in self.db.execute(stmt)}
